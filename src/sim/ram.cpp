...
uint32_t read_ram_halfword(uint32_t addr, bool is_unsigned) {
    if (addr + 1 >= avail_size) return 0x0;
...
uint32_t read_ram_word(uint32_t addr) {
    if (addr + 3 >= avail_size) return 0x0;
    uint32_t result = 0;
    for (int i = 0; i < 4; i++) {
...
bool write_ram(uint32_t addr, uint32_t data, unsigned char byte_cnt) {
    if (addr + byte_cnt - 1 >= avail_size) return false;
    uint32_t mask = 0xFF;
    for (int i = 0; i < byte_cnt; i++) {
        ram[addr + i] = (data & mask) >> (8 * i);
        mask <<= 8;
    }
    return true;
}
...
uint8_t *get_ram_ptr(uint32_t addr) {
    if (addr > avail_size) return NULL;
    return &ram[addr];
}
