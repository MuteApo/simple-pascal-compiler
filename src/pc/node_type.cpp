...
TypeAttrNode::TypeAttrNode(void) { this->root_type = pointer; }

TypeAttrNode::TypeAttrNode(std::string id) {
    this->root_type = type_identifier;
    this->type_id = id;
}

TypeAttrNode::TypeAttrNode(BasicAttrNode *attr_node) {
    this->root_type = basic;
    this->basic_attr = attr_node;
}

TypeAttrNode::TypeAttrNode(OrdAttrNode *attr_node) {
    this->root_type = ordinal;
    this->ord_attr = attr_node;
}

TypeAttrNode::TypeAttrNode(StructAttrNode *attr_node) {
    this->root_type = structured;
    this->struct_attr = attr_node;
}
...
int TypeAttrNode::get_length(void) {
    ...
    case pointer:
        return BASIC_PTR_LEN;
    case type_identifier:
        if (symbol_table.findSymbol(this->type_id) == nullptr)
            return -1; // Type identifer not found in all scope
        else
            return symbol_table.findSymbol(this->type_id)->get_length();
    }
    return -1;
}
...
bool TypeAttrNode::is_type_equ(TypeAttrNode *type, bool use_struct) {
    ...
    switch (this->root_type) {
    case basic:
        return this->basic_attr->is_type_equ(type);
    case ordinal:
        return this->ord_attr->is_type_equ(type);
    case structured:
        return this->struct_attr->is_type_equ(type);
    case pointer:
        return type->root_type == pointer;
    case type_identifier: {
...
basic_type_kind BasicAttrNode::get_type(void) { return this->type; }

int BasicAttrNode::get_length(void) {
    switch (this->type) {
    case boolean:
        return BASIC_BOOL_LEN;
    case integer:
        return BASIC_INT_LEN;
    case real:
        return BASIC_REAL_LEN;
    case character:
        return BASIC_CHAR_LEN;
    case pointer:
        return BASIC_PTR_LEN;
    }
    return -1;
...
OrdAttrNode::OrdAttrNode(std::vector<std::string> ids)
    : is_subrange(false), subrange_attr(nullptr),
...
int OrdAttrNode::get_length(void) {
    return this->is_subrange ? this->subrange_attr->get_length()
                             : this->enum_attr->get_length();
}

int OrdAttrNode::get_size(void) {
    return this->is_subrange ? this->subrange_attr->get_size()
                             : this->enum_attr->get_size();
}
...
bool SubrangeAttrNode::is_type_equ(SubrangeAttrNode *type) {
    if (type->is_int_bound != this->is_int_bound)
        return false;
    return type->bounds[0] == this->bounds[0] &&
           type->bounds[1] == this->bounds[1];
}

void EnumAttrNode::addItem(std::string id) { this->identifiers.push_back(id); }

int EnumAttrNode::get_length(void) { return BASIC_INT_LEN; }

int EnumAttrNode::get_size(void) { return this->identifiers.size(); }

int EnumAttrNode::get_offset(void) { return 0; }
...
int StructAttrNode::get_length(void) {
    switch (this->type) {
    case array:
        return this->array_attr->get_length();
    case record:
        return this->record_attr->get_length();
    }
    return -1;
}
...
bool StructAttrNode::is_type_equ(TypeAttrNode *type) {
    switch (this->type) {
    case array:
        return this->array_attr->is_type_equ(type);
    case record:
        return this->record_attr->is_type_equ(type);
    }
    return false;
}

int ArrayAttrNode::get_dim() { return this->index_type.size(); }
...
int RecordAttrNode::get_dim() { return this->identifiers.size(); }

int RecordAttrNode::get_length(void) {
    int result = 0;
    for (TypeAttrNode *type : this->types)
        result += type->get_length();
    return result;
}
